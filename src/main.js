...
import { getState, setConfig, setSheetData, setSheetError, setAccessToken, getValidGuests, markSent, markUnsent, loadPersistedState } from './state.js'
...
function attachEvents() {
  document.querySelector('#google-signin-btn')?.addEventListener('click', async () => {
    try {
      ...
      uiState.signInError = 'Sign-in was cancelled or failed. Please try again.'
      render(getState(), uiState)
      attachEvents()
    }
  })
  ...
  document.querySelector('#load-sheet-btn')?.addEventListener('click', async () => {
    ...
    try {
      const result = await sheet.readSheet(spreadsheetId, gid, auth.getToken())
      const { headers, statusColIndex, timestampColIndex } = await sheet.ensureStatusColumns(
        spreadsheetId, Number(gid), result.headers, auth.getToken(), result.sheetTitle
      )
      setSheetData({
        headers,
        allRows: result.rows,
  ...
  document.querySelector('#guest-list-card')?.addEventListener('click', async (e) => {
    const actionEl = e.target.closest('[data-action]')
    if (!actionEl) return
    const action = actionEl.dataset.action
    const rowIndexStr = actionEl.dataset.rowIndex ?? actionEl.closest('[data-row-index]')?.dataset.rowIndex
    ...
    if (action === 'open-whatsapp') {
      ...
      const st = getState()
      try {
        await sheet.writeStatus(st.spreadsheetId, rowIndex, st.statusColIndex, st.timestampColIndex, true, auth.getToken(), st.sheetTitle)
        markSent(rowIndex)
        render(getState(), uiState)
        attachEvents()
      } catch (e) {
      ...
      const st = getState()
      try {
        await sheet.writeStatus(st.spreadsheetId, rowIndex, st.statusColIndex, st.timestampColIndex, false, auth.getToken(), st.sheetTitle)
        markUnsent(rowIndex)
        render(getState(), uiState)
        attachEvents()
      } catch (e) {
  ...
  document.querySelector('#next-unsent-btn')?.addEventListener('click', async () => {
    ...
    const st = getState()
    try {
      await sheet.writeStatus(st.spreadsheetId, nextGuest.rowIndex, st.statusColIndex, st.timestampColIndex, true, auth.getToken(), st.sheetTitle)
...
async function autoFetchSheet(sheetUrl, token, savedNameCol, savedPhoneCol) {
  ...
  try {
    ...
    const { headers, statusColIndex, timestampColIndex } = await sheet.ensureStatusColumns(
      spreadsheetId, Number(gid), result.headers, token, result.sheetTitle
    )
    setSheetData({
      headers,
      allRows: result.rows,
...
async function initApp() {
  ...
  try {
    ...
    uiState.signInError = 'Google Sign-In could not be loaded. Please check your internet connection and refresh.'
    render(getState(), uiState)
    attachEvents()
    return
  }

  auth.initAuth(CLIENT_ID)
